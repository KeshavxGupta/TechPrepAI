...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const isAdmin = currentUser && (currentUser.email === 'khushboo2006june@admin.com' || currentUser.role === 'admin');
  if (!isAdmin) {
  ...
  const initialTab = urlParams.get('tab') || 'quizzes';
...
async function loadQuizzes() {
  if (window.TechPrepAPI && window.TechPrepAPI.getQuizzes) {
    try {
      ...
      if (res && res.success && Array.isArray(res.quizzes) && res.quizzes.length > 0) {
        quizzes = res.quizzes.map(q => ({ ...q, id: q.customId || q._id || q.id }));
        localStorage.setItem('techprep_quizzes', JSON.stringify(quizzes));
        renderQuizzes();
        updateDashboardStats();
        return;
      }
    } catch (e) {
      console.warn('API getQuizzes fallback:', e);
    }
  }
...
async function renderAdminPlacements() {
  ...
  if (window.TechPrepAPI && window.TechPrepAPI.getPlacements) {
    try {
      const res = await window.TechPrepAPI.getPlacements();
      if (res && res.success && Array.isArray(res.placements) && res.placements.length > 0) {
        const formatted = res.placements.map(p => ({ ...p, id: p.customId || p._id || p.id }));
        localStorage.setItem('techprep_placements', JSON.stringify(formatted));
      }
    } catch (e) {
      console.warn('API getPlacements fallback:', e);
    }
  }
...
async function renderUsers() {
  if (window.TechPrepAPI && window.TechPrepAPI.getAllUsers) {
    try {
      const res = await window.TechPrepAPI.getAllUsers();
      if (res && res.success && Array.isArray(res.users) && res.users.length > 0) {
        localStorage.setItem('techprep_registered_users', JSON.stringify(res.users));
      }
    } catch (e) {
      console.warn('API getAllUsers fallback:', e);
    }
  }
  ...
  filtered.forEach((student, index) => {
    ...
    card.innerHTML = `
      ...
      <div id="detail-${index}" class="hidden pt-4 border-t border-neutral-200 dark:border-neutral-800 space-y-4">
        ...
        <!-- Academic & Contact Quick Glance -->
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-2 border-t border-neutral-200 dark:border-neutral-800/60">
            <span class="text-[10px] font-bold text-neutral-400 font-mono uppercase block">Academic Info</span>
            <p class="text-xs text-neutral-700 dark:text-neutral-300"><strong>College:</strong> ${student.college || 'N/A'}</p>
            <p class="text-xs text-neutral-700 dark:text-neutral-300"><strong>Degree/Branch:</strong> ${student.degree || 'N/A'} - ${student.branch || 'N/A'}</p>
            <p class="text-xs text-neutral-700 dark:text-neutral-300"><strong>CGPA:</strong> ${student.cgpa || 'N/A'}</p>
            <p class="text-xs text-neutral-700 dark:text-neutral-300"><strong>Grad Year:</strong> ${student.gradYear || 'N/A'}</p>
          </div>
          <div class="space-y-1">
            <span class="text-[10px] font-bold text-neutral-400 font-mono uppercase block">Contact & Links</span>
            <p class="text-xs text-neutral-700 dark:text-neutral-300"><strong>Phone:</strong> ${student.contact || 'N/A'}</p>
            <p class="text-xs text-neutral-700 dark:text-neutral-300 truncate"><strong>GitHub:</strong> <a href="${student.github || '#'}" target="_blank" class="text-blue-500 hover:underline">${student.github || 'N/A'}</a></p>
            <p class="text-xs text-neutral-700 dark:text-neutral-300 truncate"><strong>LinkedIn:</strong> <a href="${student.linkedin || '#'}" target="_blank" class="text-blue-500 hover:underline">${student.linkedin || 'N/A'}</a></p>
          </div>
          <div class="space-y-1">
            <span class="text-[10px] font-bold text-neutral-400 font-mono uppercase block">Assessment Activity</span>
            <p class="text-xs text-neutral-700 dark:text-neutral-300"><strong>Total Attempts:</strong> ${attempts.length}</p>
            <p class="text-xs text-neutral-700 dark:text-neutral-300"><strong>Member Since:</strong> ${new Date(student.createdAt || Date.now()).toLocaleDateString()}</p>
          </div>
        </div>
...
function renderUsersMetrics(students) {
  ...
  const completeCountEl = document.getElementById('chart-complete-count');
  if (completeCountEl) completeCountEl.textContent = completeCount;
  const completeBarEl = document.getElementById('chart-complete-bar');
  if (completeBarEl) completeBarEl.style.width = `${completePct}%`;
  
  const incompCountEl = document.getElementById('chart-incomplete-count');
  if (incompCountEl) incompCountEl.textContent = incompleteCount;
  const incompBarEl = document.getElementById('chart-incomplete-bar');
  if (incompBarEl) incompBarEl.style.width = `${incompletePct}%`;

  const activeCountEl = document.getElementById('chart-active-count');
  if (activeCountEl) activeCountEl.textContent = activeCount;
  const activeBarEl = document.getElementById('chart-active-bar');
  ...
  const totalAttemptsEl = document.getElementById('metrics-total-attempts');
  if (totalAttemptsEl) totalAttemptsEl.textContent = totalAttemptsCount;
}

// Administrative suspension toggler
async function toggleUserSuspension(email) {
  if (window.TechPrepAPI && window.TechPrepAPI.toggleSuspendUser) {
    const res = await window.TechPrepAPI.toggleSuspendUser(email);
    if (res.success) {
      showToast(res.message || "Status updated successfully.");
      const users = JSON.parse(localStorage.getItem('techprep_registered_users') || '[]');
      const userIdx = users.findIndex(u => u.email === email);
      if (userIdx > -1) {
        users[userIdx].suspended = !users[userIdx].suspended;
...
async function handleAdminResetPassword(email, inputId) {
  ...
  if (window.TechPrepAPI && window.TechPrepAPI.resetUserPassword) {
    const res = await window.TechPrepAPI.resetUserPassword(email, newPwd);
    if (res.success) {
      showToast("Password updated in database successfully.");
...
function updateDBTelemetry() {
  const currentQuizzes = (window.QuizStorage && window.QuizStorage.getQuizzes) ? window.QuizStorage.getQuizzes() : (quizzes || []);
  const users = JSON.parse(localStorage.getItem('techprep_registered_users') || '[]');
  const attempts = JSON.parse(localStorage.getItem('techprep_attempt_history') || '[]');
  const faqs = typeof getFAQs === 'function' ? getFAQs() : [];
  const logs = typeof getAuditLogs === 'function' ? getAuditLogs() : [];

  const totalRecords = currentQuizzes.length + users.length + attempts.length + faqs.length + logs.length;
