...
function initAuthSystem() {
  ...
  if (currentUser) {
    ...
    if (navAuthContainer) {
      if (isAdmin) {
        navAuthContainer.innerHTML = `
          <a href="admin-hub.html" class="px-3.5 py-1.5 text-xs font-semibold rounded-md border border-neutral-300 dark:border-neutral-700 text-neutral-700 dark:text-neutral-300 hover:text-neutral-900 dark:hover:text-white transition-colors">Admin Console</a>
        ...
        navAuthContainer.innerHTML = `
          <a href="dashboard.html" class="px-3.5 py-1.5 text-xs font-semibold text-neutral-700 dark:text-neutral-300 hover:text-neutral-900 dark:hover:text-white transition-colors">
            Dashboard
          </a>
          <button onclick="logoutUser()" class="px-4 py-1.5 text-xs font-semibold rounded-md bg-neutral-900 text-white dark:bg-white dark:text-neutral-950 hover:bg-neutral-800 dark:hover:bg-neutral-100 transition-colors shadow-sm">
            Logout
    ...
    if (mobileNavAuthContainer) {
      if (isAdmin) {
        mobileNavAuthContainer.innerHTML = `
          <a href="admin-hub.html" class="block text-center px-4 py-2 text-sm font-semibold rounded-md border border-neutral-200 dark:border-neutral-700 text-neutral-800 dark:text-neutral-200">Admin Console</a>
        ...
        mobileNavAuthContainer.innerHTML = `
          <a href="dashboard.html" class="block text-center px-4 py-2 text-sm font-semibold rounded-md bg-neutral-900 text-white dark:bg-white dark:text-neutral-950">
            Dashboard
          </a>
          <button onclick="logoutUser()" class="w-full text-center px-4 py-2 text-sm font-semibold rounded-md border border-rose-500/30 text-rose-600">
            Logout
    ...
    loginBtns.forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        e.preventDefault();
        window.location.href = isAdmin ? 'admin-hub.html' : 'dashboard.html';
      });
    });
  ...
  if (standaloneLoginForm) {
    standaloneLoginForm.addEventListener('submit', (e) => {
      ...
      if (matchedUser) {
        ...
        const redirectTarget = matchedUser.email === 'khushboo2006june@admin.com' ? 'admin-hub.html' : 'dashboard.html';
  ...
  if (loginModalForm) {
    loginModalForm.addEventListener('submit', (e) => {
      ...
      if (matchedUser) {
        storage.set(CURRENT_USER_KEY, matchedUser);
        window.location.href = matchedUser.email === 'khushboo2006june@admin.com' ? 'admin-hub.html' : 'dashboard.html';
      } else {
