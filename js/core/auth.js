var USERS_STORAGE_KEY = 'techprep_registered_users';
var CURRENT_USER_KEY = 'techprep_current_user';
var ADMIN_PROFILE_KEY = 'techprep_admin_profile';
...
window.loginUser = async function (email, password) {
  ...
  const isAdminCreds = (email === 'khushboo2006june@admin.com' && password === 'khushboo');

  // Try backend API first
  if (window.TechPrepAPI && window.TechPrepAPI.login) {
    try {
      const apiRes = await window.TechPrepAPI.login(email, password);
      if (apiRes && apiRes.success && apiRes.user) {
        const userObj = { ...apiRes.user, token: apiRes.token };
        localStorage.setItem(CURRENT_USER_KEY, JSON.stringify(userObj));
        return { success: true, user: userObj };
      }
      if (!isAdminCreds && !apiRes.isNetworkError) {
...
window.registerUser = async function (userData) {
  ...
  // Try backend API first
  if (window.TechPrepAPI && window.TechPrepAPI.register) {
    const apiRes = await window.TechPrepAPI.register(userData);
...
function initAuthSystem() {
  ...
  if (currentUser) {
    const isAdmin = currentUser.email === 'khushboo2006june@admin.com' || currentUser.role === 'admin';
  ...
  if (standaloneSignupForm) {
    standaloneSignupForm.addEventListener('submit', async (e) => {
      e.preventDefault();
      ...
      if (!res.success) {
        window.customAlert('Registration Error', res.message || 'Email already registered', 'warning');
        showAuthFeedback('signup-error-msg', res.message || 'Email already registered');
        return;
      }
  ...
  if (standaloneLoginForm) {
    standaloneLoginForm.addEventListener('submit', async (e) => {
      ...
      if (res.success && res.user) {
        ...
        const isAdmin = matchedUser.role === 'admin' || matchedUser.email === 'khushboo2006june@admin.com';
        const isSub = window.location.pathname.includes('/pages/');
        const redirectTarget = isAdmin ? (isSub ? '../admin/admin-hub.html' : 'pages/admin/admin-hub.html') : (isSub ? '../user/dashboard.html' : 'pages/user/dashboard.html');
        showAuthFeedback('login-error-msg', 'Login successful! Redirecting...', false);
        setTimeout(() => {
          window.location.href = redirectTarget;
        }, 600);
      } else {
        showAuthFeedback('login-error-msg', res.message || 'Invalid email or password. Please check your credentials.');
        window.customAlert('Login Error', res.message || 'Invalid email or password. Please check your credentials.', 'error');
      }
    });
  }
  ...
  if (signupModalForm) {
    signupModalForm.addEventListener('submit', async (e) => {
      ...
      if (name && email && password) {
        ...
        const res = await window.registerUser({ name, email, password, role: 'student' });
        if (!res.success) {
          window.customAlert('Registration Error', res.message || 'This email is already registered. Please log in.', 'warning');
          return;
        }
