(function() {
  ...
  <aside id="admin-sidebar" class="fixed lg:sticky top-0 left-0 z-40 w-64 h-screen border-r border-neutral-200 dark:border-neutral-800 bg-surface-primary/95 subtle-glass flex flex-col justify-between p-5 transition-transform duration-300 -translate-x-full lg:translate-x-0 shrink-0">
    <div class="space-y-6">
      ...
      <nav class="space-y-1.5 flex flex-col text-xs font-semibold text-neutral-600 dark:text-neutral-400">
        ...
        <a href="admin-dsa.html" class="flex items-center space-x-2.5 px-3 py-2.5 rounded-lg text-left transition-all w-full text-xs font-semibold hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-600 dark:text-neutral-400">
          <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" />
          </svg>
          <span>DSA Problem Studio</span>
        </a>
        <a href="admin-resume-studio.html" class="flex items-center space-x-2.5 px-3 py-2.5 rounded-lg text-left transition-all w-full text-xs font-semibold hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-600 dark:text-neutral-400">
          <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  window.handleAdminSidebarNav = function(tabName) {
    const currentPage = (window.location.pathname.split('/').pop() || 'admin-hub.html').toLowerCase();
    if (currentPage === 'admin-hub.html') {
      if (window.switchAdminTab) window.switchAdminTab(tabName);
    } else {
      window.location.href = `admin-hub.html?tab=${tabName}`;
    }
