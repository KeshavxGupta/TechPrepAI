...
(function() {
    ...
    if (layout === 'modern_sidebar') {
          ...
          <div class="col-span-4 p-5 text-white space-y-5 flex flex-col justify-between" style="background-color: ${color}">
            <div class="space-y-5">
              ...
              <div class="space-y-1.5 text-[10px] text-white/90 font-mono border-t border-white/20 pt-3">
                ...
                ${info.email ? `<div class="truncate">✉ ${escapeHTML(info.email)}</div>` : ''}
                ${info.phone ? `<div>📱 ${escapeHTML(info.phone)}</div>` : ''}
                ${info.location ? `<div>📍 ${escapeHTML(info.location)}</div>` : ''}
                ${info.linkedin ? `<div class="truncate">🔗 ${escapeHTML(info.linkedin)}</div>` : ''}
                ${info.github ? `<div class="truncate">💻 ${escapeHTML(info.github)}</div>` : ''}
              </div>
              ...
              ${currentResume.experience.map(exp => `
                <div class="mb-3 space-y-0.5">
                  <div class="flex justify-between items-baseline font-bold text-xs">
                    ...
                    <span class="text-[9px] text-gray-500 font-mono">${escapeHTML(exp.startDate)} – ${escapeHTML(exp.endDate)}</span>
                  </div>
              ...
              <div>
                ...
                ${currentResume.experience.map(exp => `
                  <div class="mb-3 space-y-0.5">
                    ...
                    <div class="text-[9px] font-mono text-gray-500">${escapeHTML(exp.startDate)} – ${escapeHTML(exp.endDate)}</div>
      ...
      previewHTML = `
        <div class="bg-white text-gray-900 shadow-2xl rounded-lg font-serif text-left min-h-[920px] p-6 space-y-4 border-4 border-double" style="border-color: ${color}">
          ...
          ${info.summary ? `
          <div>
            <h2 class="text-xs font-bold uppercase tracking-widest text-center mb-1" style="color: ${color}">— Academic & Professional Summary —</h2>
            ...
            ${currentResume.experience.map(exp => `
              <div class="mb-3 text-[10px]">
                <div class="flex justify-between font-bold text-xs">
                  ...
                  <span class="font-mono font-normal">${escapeHTML(exp.startDate)} – ${escapeHTML(exp.endDate)}</span>
                </div>
  ...
  window.addExperienceItem = function() {
    ...
    currentResume.experience.push({
      ...
      startDate: 'Jan 2026',
      endDate: 'Present',
      description: '• Developed high performance features.'
    });
    renderExperienceFormList();
    renderLiveResumePreview();
  };
  ...
  window.addProjectItem = function() {
    ...
    currentResume.projects.push({
      ...
      description: '• Built scalable features with 99.9% uptime.'
    });
    renderProjectsFormList();
    renderLiveResumePreview();
  };
  ...
  function renderATSAnalysisReport(result) {
    ...
    if (suggestionsEl) {
      suggestionsEl.innerHTML = result.matchDetails.suggestions.map(s => `
        <li class="text-xs text-neutral-700 dark:text-neutral-300">• ${escapeHTML(s)}</li>
      `).join('');
    }
  }
